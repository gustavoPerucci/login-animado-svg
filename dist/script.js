...
function onEmailInput(e) {
	...
	// validação de e-mail para desencadear efeitos
	if(curEmailIndex > 0) {
...
function onPasswordToggleChange(e) {
	setTimeout(function() {
		...
		// se a caixa de seleção estiver desativada, oculte a senha
		} else {
...
function getPosition(el) {
	...
	while (el) {
		if (el.tagName == "BODY") {
			// lidar com peculiaridades do navegador com corpo/janela/documento e rolagem de página
			...
			// para todos os outros elementos não-BODY
...
function initLoginForm() {
	...
	// caixa de seleção lidar com eventos para senha
	...
	// mova os braços para as posições iniciais
	TweenMax.set(armL, {x: -93, y: 220, rotation: 105, transformOrigin: "top left"});
	TweenMax.set(armR, {x: -93, y: 220, rotation: -105, transformOrigin: "top right"});
	
	// definir propriedade inicial da boca (corrige bug de posicionamento)
	TweenMax.set(mouth, {transformOrigin: "center center"});
	
	// ativar piscando
	startBlinking(5);
	
	// determina até onde a entrada de e-mail pode ir antes que ocorra a rolagem
	// será usado como o ponto mais distante que o avatar olhará para a direita
	emailScrollMax = email.scrollWidth;
	
	// verifica se estamos no celular/tablet, se sim então mostra a senha inicialmente
	if(isMobileDevice()) {
		password.type = "text";
